 ...
 *
 * Examples:
 *   // Vite / Rollup — use the bundled worker shipped with this package
 *   import workerUrl from 'flippy-pdf/worker?url';
 *   setWorkerSrc(workerUrl);
 *
 *   // Or point at pdfjs-dist's worker directly
...
const WORKER_SETUP_HINT =
  "Configure it once at startup with setWorkerSrc(url), or pass `workerSrc` in the Flipbook options. " +
  "Vite/Rollup: `import workerUrl from 'flippy-pdf/worker?url'; setWorkerSrc(workerUrl);`. " +
  "Webpack 5+: `setWorkerSrc(new URL('pdfjs-dist/build/pdf.worker.mjs', import.meta.url).href);`. " +
  "Or point at a self-hosted/CDN URL directly.";
...
export class Flipbook {
  constructor(options = {}) {
    this.options = {
      // `renderScale` is resolved lazily in load() so the constructor is
      // SSR-safe (no `window` access). Pass an explicit number here to
      // override.
      renderScale: null,
      flipDuration: 700,
      enableDrag: true,
      ...
      // Optional PDF.js worker URL applied per-instance. If omitted, the
      // global value set via setWorkerSrc() is used. There is no built-in
      // CDN fallback — load() throws if no worker has been configured.
      workerSrc: null,
      ...options,
    };
  ...
  async load(pdfUrl) {
    ...
    if (!pdfjsLib.GlobalWorkerOptions.workerSrc) {
      throw new Error(`Flipbook: PDF.js worker is not configured. ${WORKER_SETUP_HINT}`);
    }
    if (this.options.renderScale == null) {
      this.options.renderScale = Math.min(
        2.5,
        Math.max(1.5, (typeof window !== 'undefined' && window.devicePixelRatio) || 1.5),
      );
    }
    this._buildUI();
    ...
    try {
      ...
      this._setError('Failed to load PDF');
      throw err;
    }
    return this;
  }
  ...
  _setLoading(loading, text) {
    ...
    if (loading) this._dom.loader.classList.remove('ic-loader-error');
  }

  _setError(text) {
    this._dom.loaderText.textContent = text;
    this._dom.loader.classList.add('ic-loader-show', 'ic-loader-error');
  }
